...
> **快照成本（2026-08-22 记录）**：当前 `game::dto::snapshot` 会重建并序列化整棵树（每次交互一次）。已去除逐节点
> `parse_fen`（节点插入时缓存 `side_to_move`/`fullmove_number`）。当前棋谱规模较小，全树深序列化可接受；
> **未来棋谱规模扩大后（导入大棋谱、大量变例），应改为扁平 `id → 节点` 表 + 当前可见范围快照（增量快照）**，
> 再评估全树深序列化的取舍。不为此在当前阶段引入复杂缓存系统。
